...
namespace com.mojang.minecraft.particle
{
    public class Particle : Entity
    {
        ...
        public Particle(Level level, float x, float y, float z, float xa, float ya, float za, int tex) : base(level)
        {
            ...
            this.setPos(x, y, z);
            this.xd = xa + (float)(new Random().NextDouble() * 2.0D - 1.0D) * 0.4F;
            this.yd = ya + (float)(new Random().NextDouble() * 2.0D - 1.0D) * 0.4F;
            this.zd = za + (float)(new Random().NextDouble() * 2.0D - 1.0D) * 0.4F;
            float speed = (float)(new Random().NextDouble() + new Random().NextDouble() + 1.0D) * 0.15F;
            ...
            this.size = (float)(new Random().NextDouble() * 0.5D + 0.5D);
            this.lifetime = (int)(4.0D / (new Random().NextDouble() * 0.9D + 0.1D));
            this.age = 0;
        }
